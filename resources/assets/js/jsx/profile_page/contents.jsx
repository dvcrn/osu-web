...
var ProfileAchievementBadge = React.createClass({
	render: function() {
		var
...
var ProfileRecentAchievements = React.createClass({
	_andMore: function() {
...
var ProfileTab = React.createClass({
	...
	_changeMode: function(e) {
		e.preventDefault();
		$(document).trigger('profile:mode:select', this.props.mode);
	},

	render: function() {
		return (
			<a href='#' onClick={this._changeMode} className={this._className()}>{this.props.text}</a>
		);
	}
});

var ProfileUserPage = React.createClass({
	editLink: function() {
...
var ProfileUserPageEditor = React.createClass({
